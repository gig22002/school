...
\begin{enumerate}
	...
	\item \begin{itemize}
			...
			\item[(b)] In this scheme, an eavesdropper knows the key to the PRF $F$ ($c_0$, or $r$). Since the key is known, the attacker has full access to the PRF to evaluate. Therefore, information about the contents of $F$ might be gained via evaluation since it does not have the same properties as a hash function.
		\end{itemize}
	\item \begin{itemize}
			\item[(a)] Mal can now certify any public key, and can impersonate a legitimate actor by signing a malicious public key as someone else. They can also revoke legitimate certificates, or modify it to "un-revoke" malicious certificates that had been revoked.
			\item[(b)] Yes. If any certificate is newly revoked before Bob retrieves the CRL and after he has retrieved it previously, Bob will believe it is legitimate when checking the CRL. If the OCSP is late/ambiguous for a certificate that has actually been revoked by the CA, Bob checks the outdated CRL and sees that it is valid.
	\end{itemize}
\end{enumerate}
